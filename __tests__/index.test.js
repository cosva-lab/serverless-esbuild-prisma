...
describe('ServerlessEsbuildPrisma', () => {
  ...
  beforeEach(() => {
    ...
    mockLayerManager = {
      createLayerZip: jest.fn(() => Promise.resolve('/path/to/layer.zip')),
      handleLayerDeploymentFromZip: jest.fn(),
      getLatestLayerArn: jest.fn(() => Promise.resolve('arn:aws:lambda:us-west-2:123456789012:layer:test-layer:1')),
    ...
    CloudFormationManager.mockImplementation(() => mockCloudFormationManager);

    getSchemaWithPath.mockResolvedValue({ schemaPath: '/path/to/schema.prisma' });
  ...
  describe('constructor', () => {
    it('should initialize with correct properties', () => {
      ...
      expect(plugin.cloudFormationManager).toBe(mockCloudFormationManager);
    });
  ...
  describe('onBeforePackage', () => {
    it('should set Prisma environment variables for all functions when useLayer is true', async () => {
      ...
      expect(mockFunctionManager.setPrismaEnvironmentVariables).toHaveBeenCalledTimes(2);
      expect(mockLogger.info).toHaveBeenCalledWith('Setting Prisma environment variables for functions...');
  ...
  describe('onPackageFinalize', () => {
    it('should process functions with layer when useLayer is true', async () => {
      ...
      expect(mockLayerManager.createLayerZip).toHaveBeenCalledWith('/path/to/schema.prisma');
      expect(mockLogger.success).toHaveBeenCalledWith('Layer zip generated for deployment');
      expect(mockFunctionManager.writePrismaSchemaToZip).toHaveBeenCalledWith('function1', { prismaSchema: '/path/to/schema.prisma' });
      expect(mockFunctionManager.writePrismaSchemaToZip).toHaveBeenCalledWith('function2', { prismaSchema: '/path/to/schema.prisma' });
    });
    ...
    it('should process functions without layer when useLayer is false', async () => {
      ...
      expect(mockFunctionManager.writePrismaSchemaAndEngineToZip).toHaveBeenCalledWith('function1', { prismaSchema: '/path/to/schema.prisma' });
      expect(mockFunctionManager.writePrismaSchemaAndEngineToZip).toHaveBeenCalledWith('function2', { prismaSchema: '/path/to/schema.prisma' });
    });
  });
  ...
  describe('onBeforeDeploy', () => {
    it('should skip when useLayer is false', async () => {
      ...
      expect(mockLayerManager.handleLayerDeploymentFromZip).not.toHaveBeenCalled();
    });
    ...
    it('should handle layer deployment when useLayer is true and not processed', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('Starting Prisma layer deployment process');
      expect(mockLayerManager.createLayerZip).toHaveBeenCalledWith('/path/to/schema.prisma');
      expect(mockLayerManager.handleLayerDeploymentFromZip).toHaveBeenCalledWith('/path/to/layer.zip');
      expect(plugin.deployProcessed).toBe(true);
      expect(mockLogger.success).toHaveBeenCalledWith('Prisma layer deployment process completed');
    });
  });
  ...
  describe('onAfterDeploy', () => {
    it('should skip when useLayer is false', async () => {
      ...
      expect(mockLogger.info).not.toHaveBeenCalledWith(expect.stringContaining('Updating functions'));
    });
    ...
    it('should update functions with latest layer when useLayer is true', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('Updating functions with latest layer version...');
      expect(plugin.updateFunctionsWithLatestLayer).toHaveBeenCalled();
      expect(mockLogger.success).toHaveBeenCalledWith('Functions updated with latest layer version');
    });
  });
  ...
  describe('onBeforeMergeCustomResources', () => {
    it('should remove layers from template when useLayer is false', async () => {
      ...
      expect(mockCloudFormationManager.removeLayersFromTemplate).toHaveBeenCalled();
    });
  ...
  describe('handleLayerAssignment', () => {
    it('should add layers to template with existing layer ARN', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('Adding layers to CloudFormation template...');
      expect(mockLayerManager.getLatestLayerArn).toHaveBeenCalledWith('test-layer');
      expect(mockCloudFormationManager.addLayersToTemplate).toHaveBeenCalledWith('arn:aws:lambda:us-west-2:123456789012:layer:test-layer:1');
      expect(mockLogger.success).toHaveBeenCalledWith(expect.stringContaining('Added layer to CloudFormation template'));
    });
    ...
    it('should add placeholder layer ARN when no existing layer', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('No existing layer found, adding placeholder for first-time deployment');
      expect(mockCloudFormationManager.addLayersToTemplate).toHaveBeenCalledWith('arn:aws:lambda:us-west-2:123456789012:layer:test-layer:1');
    });

    it('should handle errors during layer assignment', async () => {
      mockLayerManager.getLatestLayerArn.mockRejectedValue(new Error('Layer error'));

      await plugin.handleLayerAssignment();

      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining('Error adding layers to CloudFormation template'));
    });
  });
  ...
  describe('updateFunctionsWithLatestLayer', () => {
    it('should update functions with latest layer ARN', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('Using latest layer ARN'));
      expect(mockCloudFormationManager.updateLayersInTemplate).toHaveBeenCalledWith('arn:aws:lambda:us-west-2:123456789012:layer:test-layer:1');
      expect(mockFunctionManager.updateFunctionsWithLayer).toHaveBeenCalledWith(
    ...
    it('should warn when no layer versions found', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith('No layer versions found after deployment');
    });

    it('should handle errors during function update', async () => {
      mockLayerManager.getLatestLayerArn.mockRejectedValue(new Error('Update error'));

      await plugin.updateFunctionsWithLatestLayer();

      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining('Error updating function layers'));
    });
  });
});
