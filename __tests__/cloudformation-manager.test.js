...
describe('CloudFormationManager', () => {
  ...
  beforeEach(() => {
        ...
        aws: {
          naming: {
            getLambdaLogicalId: jest.fn(
              functionName => `${functionName}LambdaFunction`,
            ),
          },
        },
      },
    ...
    cloudFormationManager = new CloudFormationManager(
      mockServerless,
      mockConfig,
      mockLogger,
    );
  ...
  describe('addLayersToTemplate', () => {
    it('should call modifyTemplate with add operation', async () => {
      ...
      await cloudFormationManager.addLayersToTemplate(
        'arn:aws:lambda:us-west-2:123456789012:layer:test-layer:1',
      );

      expect(
        cloudFormationManager.modifyTemplate,
  ...
  describe('removeLayersFromTemplate', () => {
    it('should call modifyTemplate with remove operation', async () => {
      ...
      expect(
        cloudFormationManager.modifyTemplate,
      ).toHaveBeenCalledWith('remove', null);
    });
  });
  ...
  describe('updateLayersInTemplate', () => {
    it('should call modifyTemplate with update operation', async () => {
      ...
      await cloudFormationManager.updateLayersInTemplate(
  ...
  describe('modifyTemplate', () => {
    it('should handle missing compiled template', async () => {
      ...
      await cloudFormationManager.modifyTemplate(
        'add',
        'test-layer-arn',
      );

      expect(mockLogger.warn).toHaveBeenCalledWith(
        'No compiled CloudFormation template found',
      );
    });
    ...
    it('should process all functions for add operation', async () => {
      ...
      expect(
        cloudFormationManager.addLayerToFunctionResource,
      ).toHaveBeenCalledWith(
        mockServerless.service.provider.compiledCloudFormationTemplate
          .Resources['Function1LambdaFunction'],
        'Function1LambdaFunction',
        'test-layer-arn',
      );
      expect(
        ...
        mockServerless.service.provider.compiledCloudFormationTemplate
          .Resources['Function2LambdaFunction'],
        'Function2LambdaFunction',
        'test-layer-arn',
      );
    });
    ...
    it('should process all functions for remove operation', async () => {
      cloudFormationManager.removeLayersFromFunctionResource =
        jest.fn();
    ...
    it('should handle missing function resources', async () => {
      mockServerless.providers.aws.naming.getLambdaLogicalId.mockReturnValue(
        'NonExistentFunction',
      );
      cloudFormationManager.addLayerToFunctionResource = jest.fn();
    ...
    it('should handle errors and rethrow', async () => {
      ...
      await expect(
        cloudFormationManager.modifyTemplate('add', 'test-layer-arn'),
      ).rejects.toThrow('Config error');
      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.stringContaining(
          'Error modifying CloudFormation template',
        ),
      );
    });
  });
  ...
  describe('addLayerToFunctionResource', () => {
    it('should add layer to function resource', () => {
      ...
      expect(functionResource.Properties.Layers).toContain(
        'new-layer-arn',
      );
      expect(functionResource.Properties.Layers).toContain(
        'existing-layer',
    ...
    it('should initialize Properties and Layers if not exist', () => {
      ...
      expect(functionResource.Properties.Layers).toStrictEqual([
        'new-layer-arn',
      ]);
    });
