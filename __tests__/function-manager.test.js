...
describe('FunctionManager', () => {
  ...
  describe('updateFunctionsWithLayer', () => {
    ...
    it('should add layer to functions', async () => {
      await functionManager.updateFunctionsWithLayer(
      ...
      expect(mockLogger.success).toHaveBeenCalledWith(
        expect.stringContaining('Added layer function1'),
      );
      expect(mockLogger.success).toHaveBeenCalledWith(
        expect.stringContaining('Added layer function2'),
      );
    });
    ...
    it('should remove existing Prisma layers before adding new one', async () => {
      const mockFunction = {
        handler: 'src/handler.handler',
        layers: [
          'arn:aws:lambda:us-west-2:123456789012:layer:old-prisma-layer:1',
          'other-layer',
        ],
      };
      ...
      await functionManager.updateFunctionsWithLayer(
        'arn:aws:lambda:us-west-2:123456789012:layer:new-prisma-layer:1',
        'Updated layer',
      );

      expect(mockFunction.layers).toStrictEqual([
        'other-layer',
        'arn:aws:lambda:us-west-2:123456789012:layer:new-prisma-layer:1',
      ]);
    });
  ...
  describe('setPrismaEnvironmentVariables', () => {
    it('should set all Prisma environment variables', () => {
      ...
      expect(
        mockFunction.environment.PRISMA_QUERY_ENGINE_LIBRARY,
      ).toBe('/opt/nodejs/libquery_engine.so.node');
      expect(
        mockFunction.environment.PRISMA_QUERY_ENGINE_BINARY,
      ).toBe('/opt/nodejs/query-engine');
      expect(
        mockFunction.environment.PRISMA_MIGRATION_ENGINE_BINARY,
      ).toBe('/opt/nodejs/migration-engine');
      expect(
        mockFunction.environment.PRISMA_INTROSPECTION_ENGINE_BINARY,
      ).toBe('/opt/nodejs/introspection-engine');
      expect(mockFunction.environment.PRISMA_FMT_BINARY).toBe(
        '/opt/nodejs/prisma-fmt',
      );
    });
    ...
    it('should log debug information for each engine', () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining('Set query engine library'),
      );
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining('Set query engine binary'),
      );
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining('Set migration engine'),
      );
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining('Set introspection engine'),
      );
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining('Set prisma-fmt'),
      );
    });
  });
    ...
    it('should handle errors during zip operations', () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining(
          'Error writing to zip for function function1',
        ),
      );
    });
  });
