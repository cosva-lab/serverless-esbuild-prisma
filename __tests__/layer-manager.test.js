...
const { LambdaClient, PublishLayerVersionCommand, ListLayerVersionsCommand } = require('@aws-sdk/client-lambda');
...
describe('LayerManager', () => {
  ...
  describe('getAccountId', () => {
    it('should return account ID from STS', async () => {
      ...
      expect(mockStsClient.send).toHaveBeenCalledWith(expect.any(GetCallerIdentityCommand));
    });
  ...
  describe('createLayerZip', () => {
    ...
    it('should create layer zip with engines and schema', async () => {
      ...
      const result = await layerManager.createLayerZip('/path/to/schema.prisma');
  ...
  describe('layerExists', () => {
    ...
    it('should return false on error', async () => {
      ...
      const result = await layerManager.layerExists('test-layer');
      expect(result).toBe(false);
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining('Error checking if layer exists'));
    });
  });

  describe('layerNeedsUpdate', () => {
    beforeEach(() => {
      layerManager.calculateLayerContentHash = jest.fn(() => 'new-hash');
      layerManager.getCurrentLayerHash = jest.fn(() => Promise.resolve('old-hash'));
    });
    ...
    it('should return true when no versions exist', async () => {
      ...
      const result = await layerManager.layerNeedsUpdate('/path/to/zip', 'test-layer');
      expect(result).toBe(true);
    });
    ...
    it('should return true when content hash differs', async () => {
      ...
      const result = await layerManager.layerNeedsUpdate('/path/to/zip', 'test-layer');
      expect(result).toBe(true);
      expect(layerManager.calculateLayerContentHash).toHaveBeenCalled();
      expect(layerManager.getCurrentLayerHash).toHaveBeenCalledWith('test-layer', 1);
    });

    it('should return false when content hash matches', async () => {
      layerManager.getCurrentLayerHash = jest.fn(() => Promise.resolve('new-hash'));
      mockLambdaClient.send.mockResolvedValue({
        LayerVersions: [{ Version: 1 }]
      });
    ...
    it('should return true on error', async () => {
      ...
      const result = await layerManager.layerNeedsUpdate('/path/to/zip', 'test-layer');
      expect(result).toBe(true);
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining('Error checking layer update'));
    });
  });
  ...
  describe('calculateLayerContentHash', () => {
    ...
    it('should calculate hash from engine paths', () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining('Calculated hash for 2 engines'));
    });
  });
  ...
  describe('getCurrentLayerHash', () => {
    it('should extract hash from layer description', async () => {
      ...
      const result = await layerManager.getCurrentLayerHash('test-layer', 1);
  ...
  describe('uploadLayer', () => {
    beforeEach(() => {
      ...
      layerManager.calculateLayerContentHash = jest.fn(() => 'test-hash');
    });

    it('should upload layer successfully', async () => {
      mockLambdaClient.send.mockResolvedValue({
        Version: 1,
        LayerArn: 'arn:aws:lambda:us-west-2:123456789012:layer:test-layer:1'
      });

      const result = await layerManager.uploadLayer('/path/to/zip', 'test-layer', 'Test description');

      expect(mockLambdaClient.send).toHaveBeenCalled();
      expect(mockLogger.success).toHaveBeenCalledWith(expect.stringContaining('Layer uploaded successfully'));
      expect(result.Version).toBe(1);
    });

    it('should throw error on upload failure', async () => {
      mockLambdaClient.send.mockRejectedValue(new Error('Upload failed'));

      await expect(layerManager.uploadLayer('/path/to/zip', 'test-layer', 'Test description'))
        .rejects.toThrow('Upload failed');
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining('Error uploading layer'));
    });
  });
  ...
  describe('handleLayerDeploymentFromZip', () => {
    ...
    it('should upload layer when it does not exist', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('Processing Prisma layer'));
      expect(layerManager.uploadLayer).toHaveBeenCalled();
      expect(mockLogger.success).toHaveBeenCalledWith('Layer uploaded successfully');
    });
    ...
    it('should skip upload when layer does not need update', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('Layer content unchanged, skipping upload');
    });
  });
  ...
  describe('getLatestLayerArn', () => {
    it('should return latest layer ARN when versions exist', async () => {
      mockLambdaClient.send.mockResolvedValue({
        LayerVersions: [
          { Version: 2, LayerArn: 'arn:aws:lambda:us-west-2:123456789012:layer:test-layer:2' },
          { Version: 1, LayerArn: 'arn:aws:lambda:us-west-2:123456789012:layer:test-layer:1' }
        ]
      });

      const result = await layerManager.getLatestLayerArn('test-layer');
      expect(result).toBe('arn:aws:lambda:us-west-2:123456789012:layer:test-layer:2');
    });

    it('should construct ARN when LayerArn not provided', async () => {
      layerManager.getAccountId = jest.fn(() => Promise.resolve('123456789012'));
      mockLambdaClient.send.mockResolvedValue({
        LayerVersions: [
          { Version: 1 }
        ]
      ...
      const result = await layerManager.getLatestLayerArn('test-layer');
      expect(result).toBe('arn:aws:lambda:us-west-2:123456789012:layer:test-layer:1');
    });
    ...
    it('should return null on error', async () => {
      ...
      const result = await layerManager.getLatestLayerArn('test-layer');
      expect(result).toBeNull();
      expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining("Layer doesn't exist yet"));
    });
  });
});
